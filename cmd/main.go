...
func init() {
	...
	utilruntime.Must(glbv1alpha1.AddToScheme(scheme))
...
func main() {
	...
	mgrOptions := ctrl.Options{
		...
		LeaderElectionID:       "ecf5b57b.antware.xyz",
	...
	healthCache := haproxy.NewHealthCache()
	...
	if err := haproxyMonitor.SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to setup the backend health watcher")
		os.Exit(1)
	}
	...
	if err := (&controller.GlobalLoadBalancerReconciler{
		...
		setupLog.Error(err, "Failed to create controller", "controller", "globalloadbalancer")
		os.Exit(1)
	}
